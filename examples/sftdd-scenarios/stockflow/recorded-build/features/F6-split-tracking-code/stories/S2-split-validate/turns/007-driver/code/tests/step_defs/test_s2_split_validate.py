...
def step_seed_valid_rows(_s2_session) -> None:
    ...
    # Seed two rows with fully-parsed three-segment codes
    for i in range(1, 3):
        sess.execute(
            text(
                "INSERT INTO stock"
                " (sku, location, quantity, inventory_code, batch_number, serial_number)"
...
def step_seed_bad_rows(_s2_session) -> None:
    ...
    # 'A12': one-segment code -> batch_number NULL, serial_number NULL
    sess.execute(
        text(
            "INSERT INTO stock"
            " (sku, location, quantity, inventory_code, batch_number, serial_number)"
            " VALUES (:sku, :loc, 1, :ic, NULL, NULL)"
            " ON CONFLICT (sku, location) DO UPDATE"
            "   SET inventory_code = EXCLUDED.inventory_code,"
            "       batch_number   = NULL,"
            "       serial_number  = NULL"
        ),
        {"sku": "S2-BAD-001", "loc": "LOC-S2-BAD-001", "ic": "A12"},
    )
    # 'c': single-character code -> batch_number NULL, serial_number NULL
    sess.execute(
        text(
            "INSERT INTO stock"
            " (sku, location, quantity, inventory_code, batch_number, serial_number)"
        ...
        {"sku": "S2-BAD-002", "loc": "LOC-S2-BAD-002", "ic": "c"},
    )
    sess.commit()
