Feature: S3 Split Inventory Code - Drop the Old inventory_code Column
  ...
  Scenario: T27 serial_number column is preserved after the S3 down migration
    When the S3 down migration has been run
    Then the stock table still has the "serial_number" column owned by the S1 split migration

  Scenario: T28 inventory_code carries a NOT NULL constraint after the S3 down migration
    When the S3 down migration has been run
    Then the stock table inventory_code column carries a NOT NULL constraint that rejects a NULL insert
