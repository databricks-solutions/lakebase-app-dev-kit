...
def seed_record_with_null_optional_field_t22(db_session, sku_context) -> None:
    """Seed a record whose tracking_code is empty (the optional detail-view field).

    inventory_code was dropped by the S3 migration; the detail view's optional
    field is now tracking_code, which must render as 'not tracked' rather than
    leaving the cell blank or crashing (AC3-optional-fields-show-not-tracked, NFR-F1-6).
    """
    ...
    db_session.execute(
        text(
            ...
            " VALUES (:sku, :location, 1, '', NOW())"
            " ON CONFLICT (sku, location) DO UPDATE SET tracking_code = ''"
        ),
