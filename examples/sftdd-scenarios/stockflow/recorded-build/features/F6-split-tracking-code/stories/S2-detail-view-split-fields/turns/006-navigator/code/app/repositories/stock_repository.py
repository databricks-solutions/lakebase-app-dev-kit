...
def upsert_stock_record(
    ...
    batch_number/serial_number in place (last-write-wins,
    NFR-F1-sku-location-uniqueness) while leaving the original
    `created_at`/`actor` untouched (immutable audit fields,
    NFR-F1-durability-migrations). Never creates a second row: resolved by
    an existence check plus the DB's composite UNIQUE(sku, location)
    constraint as the write-time backstop.
    """
    session = SessionLocal()
    try:
        existing = (
