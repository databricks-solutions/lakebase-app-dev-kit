...
def seed_record_with_inventory_code(db_session) -> None:
    """INSERT with inventory_code column (nullable, added by S2 migration).
    Fails RED until the migration adds the column."""
    db_session.execute(
        text(
            "INSERT INTO stock"
            "  (sku, location, quantity, tracking_code, inventory_code, created_at)"
            " VALUES (:sku, :location, :quantity, '', :inventory_code, NOW())"
...
def row_shows_all_column_values(page: Page) -> None:
    ...
    expect(row).to_contain_text("IC-T15")
...
def seed_record_with_null_inventory_code(db_session) -> None:
    """INSERT with inventory_code = NULL (nullable column from S2 migration).
    Fails RED until the migration adds the column."""
    db_session.execute(
        text(
            "INSERT INTO stock"
            "  (sku, location, quantity, tracking_code, inventory_code, created_at)"
            " VALUES (:sku, :location, :quantity, '', NULL, NOW())"
            " ON CONFLICT (sku, location) DO UPDATE SET"
            "  quantity = EXCLUDED.quantity,"
            "  inventory_code = NULL"
        ),
