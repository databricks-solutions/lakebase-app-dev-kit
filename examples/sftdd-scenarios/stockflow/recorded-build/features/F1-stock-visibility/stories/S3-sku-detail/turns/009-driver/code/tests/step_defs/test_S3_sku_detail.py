...
def _seed(detail_context, client, location, quantity, inventory_code):
    ...
    detail_context["records"].append(
        {"location": location, "quantity": quantity, "inventory_code": inventory_code}
    )


@given("a SKU seeded with stock at two different locations")
def _seed_two_locations(detail_context, client):
    for location, quantity, inventory_code in (
        (f"LOC-{uuid.uuid4().hex[:8]}", 5, "INV-A"),
        (f"LOC-{uuid.uuid4().hex[:8]}", 12, "INV-B"),
    ):
        _seed(detail_context, client, location, quantity, inventory_code)


@given("a SKU seeded with stock whose par level is not tracked")
def _seed_untracked_par_level(detail_context, client):
    _seed(detail_context, client, f"LOC-{uuid.uuid4().hex[:8]}", 7, "INV-C")
...
def _assert_entry_inventory_code(detail_context):
    entries = detail_context["response"].json()["entries"]
    seeded = {(r["location"], r["inventory_code"]) for r in detail_context["records"]}
    returned = {(e["location"], e["inventory_code"]) for e in entries}
    assert returned == seeded, (
        f"expected each seeded (location, inventory_code) to appear in the "
        f"detail entries; seeded={seeded} returned={returned}"
    )
