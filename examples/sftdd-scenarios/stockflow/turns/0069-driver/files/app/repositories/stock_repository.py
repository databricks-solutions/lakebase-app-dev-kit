...
def count_unparseable_codes(session: Session) -> int:
    ...
    A row is considered unparseable when its inventory_code could not be split
    into three segments (location-batch-serial), leaving batch_number IS NULL
    OR serial_number IS NULL.  DBA can read this scalar before dropping legacy
    columns.
    """
    row = session.execute(
        text(
