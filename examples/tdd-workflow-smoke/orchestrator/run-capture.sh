...
# recorded replay corpus. Answer Y (write to LAKEBASE_TDD_GATE_ANSWER_FILE) to
...
#
# Recording: set LAKEBASE_TDD_RECORD_DIR to a PERSISTENT path (NOT under the
...
#
# Pausing/holding: set LAKEBASE_TDD_GATE_ANSWER_FILE to a control file. Leave it
# ABSENT to HOLD at the navigator gate; write Y to it to RESUME into the build.
#
# Usage:
#   LAKEBASE_TDD_RECORD_DIR=<dir> LAKEBASE_TDD_GATE_ANSWER_FILE=<file> \
...
# Quality model config for the LIVE capture , the SAME per-role models as
# lakebase-demo/stockflow (architect-reviewer + product-owner on opus, the rest
# on sonnet, spec-author on haiku). Overridable via the AGENT_MODELS env var.
export AGENT_MODELS="${AGENT_MODELS:-spec-author=haiku architect-reviewer=opus test-strategist=sonnet ux-designer=sonnet navigator=sonnet driver=sonnet product-owner=opus release-engineer=sonnet}"
