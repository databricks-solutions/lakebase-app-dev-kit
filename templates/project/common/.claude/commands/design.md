...
**Per-story pipeline (FEIP-7565):** the four phases above run **per story**, streamed, not as one per-feature batch. The Spec Author drafts one story, the Architect annotates that story's ACs, the Test Strategist orders its tests, and the story passes its **own per-story spec gate** (the HIL approves it into the sprint). The Scrum-Master then enqueues that story on the single build lane's ready queue (and `/build` starts it when the lane is free) while the design phases proceed on the next story. Build never waits for the whole feature-spec.
