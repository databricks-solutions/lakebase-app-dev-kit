...
2. You do not drive this by hand: the deterministic orchestrator (`lakebase-tdd-drive`, run by the `/plan` -> `/design` -> `/build` -> `/deploy` commands) routes every phase, spawning the role agents under `skills/lakebase-tdd-workflows/agents/` and surfacing the HITL gates (`spec` / `plan` / `test_list` / `promote` / `deploy`) for approval.
