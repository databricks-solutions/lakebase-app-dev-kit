...
init_esm_shims();
import { existsSync as existsSync32, readdirSync as readdirSync16, statSync as statSync10 } from "fs";
import { join as join36 } from "path";

// scripts/sftdd/story-pipeline.ts
init_esm_shims();
import { existsSync as existsSync30, readFileSync as readFileSync18, writeFileSync as writeFileSync17, mkdirSync as mkdirSync15, readdirSync as readdirSync14, statSync as statSync8, rmSync as rmSync4 } from "fs";

// scripts/sftdd/gate-conformance-guard.ts
init_esm_shims();
import { existsSync as existsSync29, readFileSync as readFileSync17, readdirSync as readdirSync13, statSync as statSync7 } from "fs";
...
var SCHEMA_DIR = join33(__dirname, "schemas");
...
init_esm_shims();
import { existsSync as existsSync31, mkdirSync as mkdirSync16, readdirSync as readdirSync15, readFileSync as readFileSync19, statSync as statSync9, writeFileSync as writeFileSync18 } from "fs";
import { join as join35 } from "path";
function listSpikes(sftddDir) {
  const root = join35(sftddDir, "spikes");
  if (!existsSync31(root)) return [];
  const out = [];
  for (const slug of readdirSync15(root)) {
    const dir = join35(root, slug);
    if (!statSync9(dir).isDirectory()) continue;
    const branchFile = join35(dir, "branch.txt");
    if (!existsSync31(branchFile)) continue;
    out.push({
      spike_slug: slug,
...
function listPipelineFeatures(sftddDir) {
  ...
  if (!existsSync32(featuresDir2)) return [];
  return readdirSync16(featuresDir2).filter((d) => statSync10(join36(featuresDir2, d)).isDirectory()).filter((d) => existsSync32(join36(featuresDir2, d, "pipeline.json"))).sort();
}
...
async function commitAllIfChanged(args) {
  ...
  for (const inc of args.include ?? []) {
    if (existsSync36(join40(args.cwd, inc))) {
