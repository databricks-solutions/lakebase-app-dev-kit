...
function extractZipToDir(zipBuffer, targetDir) {
  ...
  const sourceDir = entries.length === 1 && fs2.statSync(path2.join(tempDir, entries[0])).isDirectory() ? path2.join(tempDir, entries[0]) : tempDir;
...
function findTemplatesDir() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs4.existsSync(path3.join(candidate, "common", ".gitignore.base"))) {
...
function clearScaffoldArtifacts(targetDir) {
  ...
  for (const entry of fs4.readdirSync(targetDir)) {
    ...
    fs4.rmSync(path3.join(targetDir, entry), { recursive: true, force: true });
  }
}
...
async function deployClaudeCommands(targetDir, opts) {
  ...
  for (const entry of fs6.readdirSync(src)) {
    ...
    const before = fs6.readFileSync(path5.join(src, entry), "utf-8");
...
function kitVersion(opts) {
  try {
    const kitRoot = path5.dirname(path5.dirname(templatesRoot(opts)));
    const raw = fs6.readFileSync(path5.join(kitRoot, "package.json"), "utf-8");
...
async function installHooks(targetDir) {
  ...
  if (!fs6.existsSync(path5.join(targetDir, ".git"))) {
...
async function deployEnvExample(targetDir, args = {}) {
  fs6.writeFileSync(path5.join(targetDir, ".env.example"), renderEnvFromTemplate(args));
}
async function deployEnv(targetDir, args = {}) {
  fs6.writeFileSync(path5.join(targetDir, ".env"), renderEnvFromTemplate(args));
}
...
async function deployVscodeSettings(targetDir, opts) {
  ...
  fs6.mkdirSync(destDir, { recursive: true });
  fs6.copyFileSync(src, path5.join(destDir, "settings.json"));
}
async function deployGitignore(targetDir, language = "java", opts) {
  const base = fs6.readFileSync(path5.join(commonDir(opts), ".gitignore.base"), "utf-8");
  ...
  fs6.writeFileSync(path5.join(targetDir, ".gitignore"), base + "\n" + extra);
}
...
function adoptTdd(args) {
  ...
  for (const rel of entries) {
    ...
    if (!fs7.existsSync(toPath)) {
      if (!args.dryRun) {
        fs7.mkdirSync(path6.dirname(toPath), { recursive: true });
...
function writePlaywrightTemplates(args) {
  ...
  for (const rel of PLAYWRIGHT_TEMPLATE_FILES) {
    ...
    fs8.mkdirSync(path7.dirname(to), { recursive: true });
...
async function adoptLakebaseProject(args) {
  ...
  if (!fs11.existsSync(path10.join(args.projectDir, ".git"))) {
    throw new Error(
  ...
  if (args.enableTdd) {
    if (!dryRun) {
      ...
      for (const rel of result.added) {
        filesWritten.push(path10.join(".tdd", rel));
      }
    } else {
...
${block}` : block;
  fs12.mkdirSync(path11.dirname(args.envPath), { recursive: true });
...
async function uploadDirectory(args) {
  ...
  const uploadFile = async (localFile, relPath) => {
    const remotePath = `${args.workspacePath}/${relPath.split(import_node_path.sep).join("/")}`;
  ...
  const walk = async (dirAbs, dirRel) => {
    for (const entry of (0, import_node_fs.readdirSync)(dirAbs)) {
      const childAbs = (0, import_node_path.join)(dirAbs, entry);
      const childRel = dirRel ? `${dirRel}${import_node_path.sep}${entry}` : entry;
...
function runAlembic(ctx, args) {
  return new Promise((resolve2, reject) => {
    ...
    const child = (0, import_node_child_process12.spawn)(bin, args, {
      cwd: ctx.projectDir,
...
function runFlyway(ctx, args) {
  ...
  return new Promise((resolve2, reject) => {
    const child = (0, import_node_child_process13.spawn)(
      "flyway",
...
function runKnex(ctx, args) {
  return new Promise((resolve2, reject) => {
    ...
    const child = (0, import_node_child_process14.spawn)("npx", ["--no-install", "knex", "--knexfile", knexfile, ...args], {
      cwd: ctx.projectDir,
      env: { ...process.env, DATABASE_URL: ctx.dsn },
      stdio: ["ignore", "pipe", "pipe"]
    });
...
function cacheDir() {
  return path21.join(os.homedir(), ".cache", "github-actions-runner");
}
function runnersDir() {
  return path21.join(os.homedir(), ".lakebase", "runners");
}
function runnerDir(projectName) {
  return path21.join(runnersDir(), projectName);
}
...
async function ensureCachedArchive() {
  ...
  const cachedPath = path21.join(dir, RUNNER_ARCHIVE);
...
function isRunning(projectName) {
  const pidFile = path21.join(runnerDir(projectName), ".pid");
...
function getRunnerInfo(projectName) {
  ...
  const pidFile = path21.join(dir, ".pid");
  let pid;
...
function stopRunner(projectName) {
  ...
  for (const stale of ["_diag/pages", "_work/_temp", "_work/_actions"]) {
    const full = path21.join(dir, stale);
  ...
  try {
    fs22.mkdirSync(path21.join(dir, "_diag", "pages"), { recursive: true });
  } catch {
  }
}
function resetRunnerConfig(dir, projectName) {
  ...
  for (const f of stateFiles) {
    try {
      fs22.unlinkSync(path21.join(dir, f));
    } catch {
    }
  }
...
async function setupRunner(args) {
  ...
  if (child.pid) {
    fs22.writeFileSync(path21.join(dir, ".pid"), String(child.pid));
  }
...
${summary}`);
  ...
  const dir = path22.join(projectDir, ".lakebase");
  fs23.mkdirSync(dir, { recursive: true });
  const target = stateFilePath(projectDir);
  const tmp = `${target}.tmp`;
  const ordered = orderForOutput(result.value);
  fs23.writeFileSync(tmp, `${JSON.stringify(ordered, null, 2)}
`, "utf8");
...
async function createProject(input, progress) {
  ...
  const projectDir = path23.join(input.parentDir, input.projectName);
...
function layDownTddScaffold(targetDir) {
  const candidates = [
    path23.resolve(__dirname, "../../templates/tdd-bootstrap/.tdd"),
    path23.resolve(__dirname, "../../../templates/tdd-bootstrap/.tdd")
  ];
  const source = candidates.find((c) => fs24.existsSync(c));
  if (!source) {
    throw new Error(`tdd-bootstrap template not found; looked in: ${candidates.join(", ")}`);
  }
  const dest = path23.join(targetDir, ".tdd");
  if (fs24.existsSync(dest)) {
    return;
  }
  fs24.cpSync(source, dest, { recursive: true });
}
...
function dbcli6(args) {
  return (0, import_node_child_process15.execFileSync)("databricks", args, {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"],
    timeout: KIT_TIMEOUTS.cliDefault
  });
...
function listPipelineFeatures(tddDir) {
  ...
  if (!(0, import_fs5.existsSync)(featuresDir)) return [];
  return (0, import_fs5.readdirSync)(featuresDir).filter((d) => (0, import_fs5.statSync)((0, import_path5.join)(featuresDir, d)).isDirectory()).filter((d) => (0, import_fs5.existsSync)((0, import_path5.join)(featuresDir, d, "pipeline.json"))).sort();
}
...
async function runDoctor(args) {
  ...
  for (const stale of findStaleBranches(path26.join(projectDir, ".tdd"))) {
