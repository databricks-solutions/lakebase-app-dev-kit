...
function resolveAlembicBin(projectDir2) {
  const candidates = [
    path.join(projectDir2, ".venv", "bin", "alembic"),
    path.join(projectDir2, "venv", "bin", "alembic")
  ];
...
function findVersionsDir(projectDir2) {
  const candidates = [
    path2.join(projectDir2, "migrations", "versions"),
    path2.join(projectDir2, "alembic", "versions")
  ];
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir2) {
    if (fs2.existsSync(path2.join(projectDir2, "alembic.ini"))) return true;
    if (fs2.existsSync(path2.join(projectDir2, "migrations", "env.py"))) return true;
    if (fs2.existsSync(path2.join(projectDir2, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
function migrationsLocation(projectDir2) {
  return `filesystem:${path3.join(projectDir2, "src", "main", "resources", "db", "migration")}`;
}
...
function listFlywayFiles(projectDir2) {
  const dir = path4.join(projectDir2, "src", "main", "resources", "db", "migration");
...
var FlywayAdapter = {
  ...
  detect(projectDir2) {
    return fs3.existsSync(path4.join(projectDir2, "pom.xml"));
  },
  async apply(args) {
...
function findKnexfile(projectDir2) {
  for (const name of KNEXFILE_VARIANTS) {
    const p = path5.join(projectDir2, name);
...
function listKnexFiles(projectDir2) {
  const dir = path6.join(projectDir2, "migrations");
...
var KnexAdapter = {
   ...
   */
  detect(projectDir2) {
    return KNEXFILE_VARIANTS2.some((name) => fs5.existsSync(path6.join(projectDir2, name)));
  },
  async apply(args) {
...
function detectLanguage(projectDir2) {
  if (fs6.existsSync(path7.join(projectDir2, "pom.xml"))) {
    return "java";
  }
  if (fs6.existsSync(path7.join(projectDir2, "pyproject.toml")) || fs6.existsSync(path7.join(projectDir2, "requirements.txt")) || fs6.existsSync(path7.join(projectDir2, "alembic.ini"))) {
    return "python";
  }
  if (fs6.existsSync(path7.join(projectDir2, "package.json"))) {
    return "nodejs";
  }
  throw new SchemaMigrationError(
