...
function detectLanguageAt(dir) {
  ...
  if (fs.existsSync(path.join(dir, "pyproject.toml")) || fs.existsSync(path.join(dir, "requirements.txt")) || fs.existsSync(path.join(dir, "alembic.ini"))) {
    return "python";
  }
...
function resolveMigrationLanguage(projectDir2, configuredMigrationPath, override) {
  ...
  const rootResolved = path.resolve(projectDir2);
  let dir = path.resolve(projectDir2, rel);
...
function resolveAlembicBin(projectDir2) {
  const candidates = [
    path2.join(projectDir2, ".venv", "bin", "alembic"),
    path2.join(projectDir2, "venv", "bin", "alembic")
  ];
...
async function createAlembicRevision(opts) {
  ...
  for (const rel of ["migrations/versions", "alembic/versions"]) {
    ...
    const hit = fs2.readdirSync(dir).find((f) => f.startsWith(`${opts.revId}_`) && f.endsWith(".py"));
...
function findVersionsDir(projectDir2) {
  const candidates = [
    path3.join(projectDir2, "migrations", "versions"),
    path3.join(projectDir2, "alembic", "versions")
  ];
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir2) {
    if (fs3.existsSync(path3.join(projectDir2, "alembic.ini"))) return true;
    if (fs3.existsSync(path3.join(projectDir2, "migrations", "env.py"))) return true;
    if (fs3.existsSync(path3.join(projectDir2, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
function migrationsLocation(projectDir2) {
  return `filesystem:${path4.join(projectDir2, "src", "main", "resources", "db", "migration")}`;
}
...
function listFlywayFiles(projectDir2) {
  const dir = path5.join(projectDir2, "src", "main", "resources", "db", "migration");
...
var FlywayAdapter = {
  ...
  detect(projectDir2) {
    return fs4.existsSync(path5.join(projectDir2, "pom.xml"));
  },
  async apply(args) {
  ...
  async newMigration(args) {
    try {
      ...
      if (fs4.existsSync(full)) throw new Error(`${filename} already exists`);
      fs4.writeFileSync(
        full,
        `-- V${version}: ${args.slug}
...
function findKnexfile(projectDir2) {
  for (const name of KNEXFILE_VARIANTS) {
    const p = path6.join(projectDir2, name);
...
function listKnexFiles(projectDir2) {
  const dir = path7.join(projectDir2, "migrations");
...
var KnexAdapter = {
   ...
   */
  detect(projectDir2) {
    return KNEXFILE_VARIANTS2.some((name) => fs6.existsSync(path7.join(projectDir2, name)));
  },
  async apply(args) {
...
//# sourceMappingURL=detect-language.cli.js.map