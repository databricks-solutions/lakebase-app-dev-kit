...
init_esm_shims();
import { existsSync as existsSync9, readdirSync as readdirSync5, statSync as statSync5 } from "fs";
import { join as join9 } from "path";

// scripts/sftdd/story-pipeline.ts
init_esm_shims();
import { existsSync as existsSync7, readFileSync as readFileSync7, writeFileSync as writeFileSync5, mkdirSync as mkdirSync4, readdirSync as readdirSync3, statSync as statSync3, rmSync } from "fs";

// scripts/sftdd/gate-conformance-guard.ts
init_esm_shims();
import { existsSync as existsSync6, readFileSync as readFileSync6, readdirSync as readdirSync2, statSync as statSync2 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync8, mkdirSync as mkdirSync5, readdirSync as readdirSync4, readFileSync as readFileSync8, statSync as statSync4, writeFileSync as writeFileSync6 } from "fs";
import { join as join8 } from "path";
function listSpikes(sftddDir) {
  const root = join8(sftddDir, "spikes");
  if (!existsSync8(root)) return [];
  const out = [];
  for (const slug of readdirSync4(root)) {
    const dir = join8(root, slug);
    if (!statSync4(dir).isDirectory()) continue;
    const branchFile = join8(dir, "branch.txt");
    if (!existsSync8(branchFile)) continue;
    out.push({
      spike_slug: slug,
      branch_id: readFileSync8(branchFile, "utf8").trim(),
...
function listPipelineFeatures(sftddDir) {
  ...
  if (!existsSync9(featuresDir2)) return [];
  return readdirSync5(featuresDir2).filter((d) => statSync5(join9(featuresDir2, d)).isDirectory()).filter((d) => existsSync9(join9(featuresDir2, d, "pipeline.json"))).sort();
}
...
function readEnv(projectDir) {
  ...
  if (!fs13.existsSync(envPath)) return out;
