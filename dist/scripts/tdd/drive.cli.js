...
init_esm_shims();
import { existsSync as existsSync2, mkdirSync as mkdirSync2, readdirSync as readdirSync2, readFileSync as readFileSync2, writeFileSync as writeFileSync2, copyFileSync, statSync as statSync2 } from "fs";
...
function cpDir(srcDir, dstDir, transform) {
  if (!existsSync2(srcDir)) return false;
  let copied = false;
  mkdirSync2(dstDir, { recursive: true });
  for (const name of readdirSync2(srcDir)) {
    const s = join2(srcDir, name);
    if (!statSync2(s).isFile()) continue;
    const text = readFileSync2(s, "utf8");
    writeFileSync2(join2(dstDir, name), transform ? transform(name, text) : text);
    copied = true;
  }
  return copied;
}
...
function replayDesignTurn(args) {
  ...
  const cf = join2(featuresDir(replayDir), featureId);
  const tf = join2(featuresDir(tddDir), featureId);
  switch (turn.role) {
    case "spec-author": {
      if (turn.mode === "propose") {
        return cp(join2(replayDir, "planning", "feature-proposals.md"), join2(tddDir, "planning", "feature-proposals.md"));
      }
      if (turn.mode === "breakdown") {
        let ok = cp(join2(cf, "feature-spec.json"), join2(tf, "feature-spec.json"));
        cp(join2(cf, "feature-spec.md"), join2(tf, "feature-spec.md"));
        const storiesSrc = join2(cf, "stories");
        if (existsSync2(storiesSrc)) {
          for (const s of readdirSync2(storiesSrc)) {
            cp(join2(storiesSrc, s, "story.json"), join2(tf, "stories", s, "story.json"));
            cp(join2(storiesSrc, s, "story.md"), join2(tf, "stories", s, "story.md"));
          }
        }
        return ok;
      }
      if (turn.story) {
        return cpDir(join2(cf, "stories", turn.story, "acs"), join2(tf, "stories", turn.story, "acs"), stripLayer);
      }
      return false;
    }
    case "architect-reviewer": {
      let ok = cp(join2(cf, "architecture.json"), join2(tf, "architecture.json"));
      cp(join2(cf, "architecture.md"), join2(tf, "architecture.md"));
      if (turn.story) {
        const acs = cpDir(join2(cf, "stories", turn.story, "acs"), join2(tf, "stories", turn.story, "acs"));
        ok = ok || acs;
      }
      return ok;
    }
    case "test-strategist": {
      let ok = cp(join2(cf, "test-list.json"), join2(tf, "test-list.json"));
      cp(join2(cf, "test-list.md"), join2(tf, "test-list.md"));
      const story = turn.story;
      if (story) {
        cp(join2(cf, "stories", story, "test-list-per-ac.json"), join2(tf, "stories", story, "test-list-per-ac.json"));
      }
      return ok;
    }
    case "ux-designer": {
      let ok = cp(join2(replayDir, "design", "design-guide.json"), join2(tddDir, "design", "design-guide.json"));
      cp(join2(replayDir, "design", "design-guide.md"), join2(tddDir, "design", "design-guide.md"));
      cp(join2(replayDir, "design", "ia.md"), join2(tddDir, "design", "ia.md"));
      return ok;
    }
    default:
      return false;
...
function recordBuildTurn(args) {
  ...
  cpSync2(projectDir, join4(turnDir, "code"), {
    recursive: true,
    force: true,
    filter: codeTreeFilter(projectDir)
  });
  const cyclesSrc = cyclesRootDir(tddDir);
  if (existsSync4(cyclesSrc)) cpSync2(cyclesSrc, join4(turnDir, "tdd", "cycles"), { recursive: true, force: true });
  const expSrc = experimentsRootDir(tddDir);
  if (existsSync4(expSrc)) cpSync2(expSrc, join4(turnDir, "tdd", "experiments"), { recursive: true, force: true });
  return turnDir;
}
...
function readIndex(recordDir) {
  const f = join5(recordDir, "turns", "index.json");
...
function recordTurn(args) {
  ...
  const turnDir = join5(recordDir, "turns", dirName);
  mkdirSync4(join5(turnDir, "files"), { recursive: true });
  const artifactsDir = join5(recordDir, "recorded-artifacts");
  for (const rel of produced) {
    const f = cur.get(rel);
    const dst = join5(turnDir, "files", rel);
    mkdirSync4(dirname2(dst), { recursive: true });
    cpSync3(f.abs, dst);
    if (f.underTdd) {
      const mirror = join5(artifactsDir, relative(tddDir, f.abs));
      mkdirSync4(dirname2(mirror), { recursive: true });
      cpSync3(f.abs, mirror);
    }
  }
  for (const rel of deleted) {
    const abs = join5(projectDir, rel);
    if (abs.startsWith(tddDir)) {
      const mirror = join5(artifactsDir, relative(tddDir, abs));
      if (existsSync5(mirror)) rmSync(mirror, { force: true });
    }
  }
  const manifest = {
    ordinal,
  ...
  writeFileSync3(join5(turnDir, "turn.json"), JSON.stringify(manifest, null, 2) + "\n");
...
function loadSchema(name) {
  return JSON.parse(readFileSync7(join8(SCHEMA_DIR, name), "utf8"));
}
...
function logFilePath(tddDir) {
  return join9(tddDir, "agent-log.jsonl");
}
...
function readSmellsLog(tddDir) {
  const file = join10(tddDir, "smells.json");
  if (!existsSync10(file)) return { detected: [] };
...
init_esm_shims();
import { existsSync as existsSync13, readFileSync as readFileSync13, readdirSync as readdirSync7, statSync as statSync6, writeFileSync as writeFileSync9, mkdirSync as mkdirSync8 } from "fs";
...
function storyCycles(tddDir, featureId, story) {
  const base = join14(cyclesRootDir(tddDir), featureId, story);
...
init_esm_shims();
import { existsSync as existsSync16, readFileSync as readFileSync16, renameSync, unlinkSync, writeFileSync as writeFileSync12 } from "fs";
...
function gatesFilePath(tddDir, featureId) {
  return join15(requireFeatureDir(tddDir, featureId), "gates.json");
}
...
function storyCycles2(tddDir, featureId, story) {
  const base = path5.join(cyclesRootDir(tddDir), featureId, story);
  if (!fs9.existsSync(base)) return [];
  const out = [];
  for (const acDir of fs9.readdirSync(base)) {
    const dir = path5.join(base, acDir);
    let isDir = false;
    try {
    ...
    for (const f of fs9.readdirSync(dir)) {
      if (!/^cycle-\d+\.json$/.test(f)) continue;
      try {
        out.push(JSON.parse(fs9.readFileSync(path5.join(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
function readDriveContext(tddDir, featureId, projectDir) {
  ...
  const proj = projectDir ?? path5.dirname(tddDir);
  let scmState;
  try {
...
init_esm_shims();
import { existsSync as existsSync19, readFileSync as readFileSync19, writeFileSync as writeFileSync14, mkdirSync as mkdirSync12, readdirSync as readdirSync11, statSync as statSync8 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync20, readFileSync as readFileSync20, writeFileSync as writeFileSync15, mkdirSync as mkdirSync13 } from "fs";
...
function buildDriveEffects(cfg) {
  return {
    ...
    onHandback(handoff, detail) {
      ...
      try {
        fs10.mkdirSync(dirname7(file), { recursive: true });
...
init_esm_shims();
import { existsSync as existsSync22, mkdirSync as mkdirSync15, readFileSync as readFileSync22, renameSync as renameSync3, unlinkSync as unlinkSync2, writeFileSync as writeFileSync17 } from "fs";
...
function readSprintGates(sprint, opts = {}) {
  ...
  let parsed;
  try {
    parsed = JSON.parse(readFileSync22(file, "utf8"));
  } catch (err) {
    const cause = err instanceof Error ? err.message : String(err);
...
init_esm_shims();
import { existsSync as existsSync24, readFileSync as readFileSync23, writeFileSync as writeFileSync18, mkdirSync as mkdirSync16 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync25, readFileSync as readFileSync24, mkdirSync as mkdirSync17, writeFileSync as writeFileSync19 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync26, mkdirSync as mkdirSync18, readFileSync as readFileSync25, writeFileSync as writeFileSync20 } from "fs";
import { join as join20 } from "path";
var RUN_CONFIG_REL = join20(".tdd", "run-config.json");
function readKitRef(projectDir) {
  const f = join20(projectDir, ".lakebase", "kit-ref");
  if (!existsSync26(f)) return void 0;
  try {
    const v = readFileSync25(f, "utf8").trim();
...
function writeRunConfig(inputs) {
  ...
  try {
    mkdirSync18(inputs.tddDir, { recursive: true });
    writeFileSync20(join20(inputs.tddDir, "run-config.json"), body);
    const recordDir = (inputs.env ?? process.env).LAKEBASE_TDD_RECORD_DIR?.trim();
    if (recordDir) {
      mkdirSync18(recordDir, { recursive: true });
      writeFileSync20(join20(recordDir, "run-config.json"), body);
    }
  } catch {
  }
  return cfg;
...
function spawnClaudeStreaming(args, cwd) {
  return new Promise((resolve2, reject) => {
    const child = spawn2("claude", args, { cwd, stdio: ["inherit", "pipe", "inherit"] });
    const lines = [];
