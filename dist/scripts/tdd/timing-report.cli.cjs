...
// scripts/tdd/run-config.ts
var RUN_CONFIG_REL = (0, import_path4.join)(".tdd", "run-config.json");
function readRunConfig(tddDir) {
  const f = (0, import_path4.join)(tddDir, "run-config.json");
...
`);
  ...
  const tddDir = parsed.tddDir ?? (0, import_path5.join)(process.cwd(), ".tdd");
