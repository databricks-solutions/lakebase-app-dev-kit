...
// scripts/tdd/imports-clean.ts
import { execSync } from "child_process";
import { existsSync as existsSync7, renameSync } from "fs";
...
function findVersionsDir(projectDir) {
  ...
  return candidates.find((p2) => fs2.existsSync(p2));
}
...
function findKnexfile(projectDir) {
  for (const name of KNEXFILE_VARIANTS) {
    const p2 = path5.join(projectDir, name);
...
function detectLanguage(projectDir) {
  if (fs6.existsSync(path7.join(projectDir, "pom.xml"))) {
    return "java";
  }
  if (fs6.existsSync(path7.join(projectDir, "pyproject.toml")) || fs6.existsSync(path7.join(projectDir, "requirements.txt")) || fs6.existsSync(path7.join(projectDir, "alembic.ini"))) {
    return "python";
  }
  if (fs6.existsSync(path7.join(projectDir, "package.json"))) {
    return "nodejs";
  }
  throw new SchemaMigrationError(
...
function detectEntry(projectDir, lang) {
  if (lang === "python") {
    if (existsSync7(join8(projectDir, "app", "main.py"))) return "app.main";
    if (existsSync7(join8(projectDir, "main.py"))) return "main";
    return null;
  }
  if (lang === "nodejs") {
    try {
      ...
      if (pkg.main && existsSync7(join8(projectDir, pkg.main))) return `./${pkg.main}`;
    } catch {
    }
    for (const cand of ["server/index.js", "index.js", "src/index.js"]) {
      if (existsSync7(join8(projectDir, cand))) return `./${cand}`;
    }
    return null;
  }
  return null;
...
function checkImportsClean(args) {
  ...
  for (const rel of buildArtifacts) {
    const from = join8(projectDir, rel);
    if (existsSync7(from)) {
  ...
  try {
    ...
    for (const h of hidden.reverse()) {
      if (existsSync7(h.to)) renameSync(h.to, h.from);
    }
  }
}

// scripts/tdd/imports-clean.cli.ts
function parse(argv) {
