...
// scripts/tdd/experiment-args.ts
init_esm_shims();
...
init_esm_shims();
import { appendFileSync, existsSync as existsSync13, readFileSync as readFileSync8 } from "fs";
...
function loadSchema(name) {
  return JSON.parse(readFileSync7(join13(SCHEMA_DIR, name), "utf8"));
}
...
function logFilePath(tddDir) {
  return join14(tddDir, "agent-log.jsonl");
}
...
function logExperimentEvent(tddDir, event, story, reason) {
  try {
...
async function main() {
  ...
  const tddDir = args.tddDir ?? join15(process.cwd(), ".tdd");
  const projectDir = args.projectDir ?? process.cwd();
