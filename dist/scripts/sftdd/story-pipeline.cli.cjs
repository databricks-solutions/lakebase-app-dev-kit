...
function canonicalArtifactName(path13) {
  const base = (0, import_path2.basename)(path13);
  if ((0, import_path2.basename)((0, import_path2.dirname)(path13)) === "acs" && base.endsWith(".json")) return "ac.json";
  return base;
}
...
function resolveProfile(opts) {
  ...
  if (profileByEnvFile.has(cwd)) {
    ...
    fromEnvFile = readEnvVar((0, import_node_path3.join)(cwd, ".env"), "DATABRICKS_CONFIG_PROFILE");
  ...
  if (gate === "spec") {
    ...
    if ((0, import_node_fs6.existsSync)(dir)) {
      for (const f of (0, import_node_fs6.readdirSync)(dir)) {
        if (f.endsWith(".json") || f.endsWith(".md")) (0, import_node_fs6.rmSync)((0, import_node_path9.join)(dir, f), { force: true });
      }
    }
...
function applyReviseSelfHeal(args) {
  ...
  try {
    ...
    (0, import_node_fs6.mkdirSync)((0, import_node_path9.dirname)(hb), { recursive: true });
...
function stateFilePath(projectDir) {
  return path11.join(projectDir, STATE_FILE_REL);
}
...
var KIT_ROOT = path12.resolve(__dirname, "..", "..", "..");
var kitBinMap = null;
function resolveKitBinJs(bin) {
  if (kitBinMap === null) {
    try {
      const pkg = JSON.parse(fs15.readFileSync(path12.join(KIT_ROOT, "package.json"), "utf8"));
      kitBinMap = pkg.bin ?? {};
    } catch {
      kitBinMap = {};
    }
  ...
  return rel ? path12.join(KIT_ROOT, rel) : null;
}
function runKitBinSync(bin, args, cwd) {
  const js = resolveKitBinJs(bin);
  const res = js ? (0, import_node_child_process8.spawnSync)("node", [js, ...args], { cwd, stdio: "inherit" }) : (0, import_node_child_process8.spawnSync)(bin, args, { cwd, stdio: "inherit" });
