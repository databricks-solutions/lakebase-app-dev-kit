...
function selectProfileForHost(profilesJson, host) {
  ...
  const names = profiles.filter((p2) => {
...
function findVersionsDir(projectDir) {
  ...
  return candidates.find((p2) => fs5.existsSync(p2));
}
...
function findKnexfile(projectDir) {
  for (const name of KNEXFILE_VARIANTS) {
    const p2 = path7.join(projectDir, name);
    if (fs7.existsSync(p2)) return p2;
  }
  return void 0;
}
...
function migrationTimestamp(now = /* @__PURE__ */ new Date()) {
  ...
  return `${now.getUTCFullYear()}${p2(now.getUTCMonth() + 1)}${p2(now.getUTCDate())}${p2(now.getUTCHours())}${p2(now.getUTCMinutes())}${p2(now.getUTCSeconds())}`;
}
...
function detectEntry(projectDir, lang) {
  if (lang === "python") {
    if ((0, import_node_fs.existsSync)((0, import_node_path2.join)(projectDir, "app", "main.py"))) return "app.main";
    if ((0, import_node_fs.existsSync)((0, import_node_path2.join)(projectDir, "main.py"))) return "main";
    return null;
  }
  if (lang === "nodejs") {
    try {
      const pkg = JSON.parse(
        (0, import_node_child_process6.execSync)("cat package.json", { cwd: projectDir }).toString()
      );
      if (pkg.main && (0, import_node_fs.existsSync)((0, import_node_path2.join)(projectDir, pkg.main))) return `./${pkg.main}`;
    } catch {
    }
    for (const cand of ["server/index.js", "index.js", "src/index.js"]) {
      if ((0, import_node_fs.existsSync)((0, import_node_path2.join)(projectDir, cand))) return `./${cand}`;
    }
    return null;
  }
  return null;
...
var defaultImporter = ({ projectDir, lang, entry }) => {
  ...
  try {
    (0, import_node_child_process6.execSync)(command, { cwd: projectDir, stdio: ["ignore", "ignore", "pipe"] });
    return { code: 0, stderr: "" };
  } catch (err) {
    const e = err;
    return {
...
function hasUv(projectDir) {
  try {
    (0, import_node_child_process6.execSync)("uv --version", { cwd: projectDir, stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
function checkImportsClean(args) {
  ...
  for (const rel of buildArtifacts) {
    const from = (0, import_node_path2.join)(projectDir, rel);
