...
import { existsSync as existsSync10, renameSync } from "fs";
...
function detectEntry(projectDir, lang) {
  if (lang === "python") {
    if (existsSync10(join11(projectDir, "app", "main.py"))) return "app.main";
    if (existsSync10(join11(projectDir, "main.py"))) return "main";
    return null;
  }
  if (lang === "nodejs") {
    try {
      ...
      if (pkg.main && existsSync10(join11(projectDir, pkg.main))) return `./${pkg.main}`;
    } catch {
    }
    for (const cand of ["server/index.js", "index.js", "src/index.js"]) {
      if (existsSync10(join11(projectDir, cand))) return `./${cand}`;
    }
    return null;
  }
  return null;
...
function checkImportsClean(args) {
  ...
  for (const rel of buildArtifacts) {
    const from = join11(projectDir, rel);
  ...
  try {
    ...
    for (const h of hidden.reverse()) {
      if (existsSync10(h.to)) renameSync(h.to, h.from);
    }
  }
}
