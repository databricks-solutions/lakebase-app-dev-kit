...
function detectLanguage(projectDir) {
  if (fs14.existsSync(path15.join(projectDir, "pom.xml"))) {
    return "java";
  }
  if (fs14.existsSync(path15.join(projectDir, "pyproject.toml")) || fs14.existsSync(path15.join(projectDir, "requirements.txt")) || fs14.existsSync(path15.join(projectDir, "alembic.ini"))) {
    return "python";
  }
  if (fs14.existsSync(path15.join(projectDir, "package.json"))) {
    return "nodejs";
  }
  throw new SchemaMigrationError(
...
function listFlywayMigrations(projectDir) {
  const dir = path15.join(projectDir, "src", "main", "resources", "db", "migration");
...
function listKnexMigrations(projectDir) {
  const dir = path15.join(projectDir, "migrations");
  if (!fs14.existsSync(dir)) return [];
