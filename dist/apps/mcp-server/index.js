...
function branchNameFromResourcePath(path25) {
  if (!path25.includes("/branches/")) return null;
  const leaf = path25.split("/branches/").pop();
  if (!leaf) return null;
  try {
...
${block}` : block;
  fs.mkdirSync(path2.dirname(args.envPath), { recursive: true });
...
function verifyHooks(projectDir) {
  ...
  return {
    postCheckout: fs2.existsSync(path3.join(hooksDir, "post-checkout")),
    prepareCommitMsg: fs2.existsSync(path3.join(hooksDir, "prepare-commit-msg")),
    prePush: fs2.existsSync(path3.join(hooksDir, "pre-push"))
  };
}
function verifyWorkflows(projectDir) {
  const wfDir = path3.join(projectDir, ".github", "workflows");
  return {
    pr: fs2.existsSync(path3.join(wfDir, "pr.yml")),
    merge: fs2.existsSync(path3.join(wfDir, "merge.yml"))
  };
}
...
function extractZipToDir(zipBuffer, targetDir) {
  ...
  const sourceDir = entries.length === 1 && fs4.statSync(path5.join(tempDir, entries[0])).isDirectory() ? path5.join(tempDir, entries[0]) : tempDir;
...
function clearScaffoldArtifacts(targetDir) {
  ...
  for (const entry of fs6.readdirSync(targetDir)) {
    ...
    fs6.rmSync(path6.join(targetDir, entry), { recursive: true, force: true });
  }
}
...
function findTemplatesDir2() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs7.existsSync(path7.join(candidate, "common", ".gitignore.base"))) {
...
function findTemplatesDir3() {
  ...
  const here = path8.dirname(fileURLToPath4(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
...
async function deployClaudeCommands(targetDir, opts) {
  ...
  for (const entry of fs8.readdirSync(src)) {
    ...
    const before = fs8.readFileSync(path8.join(src, entry), "utf-8");
...
function kitVersion(opts) {
  try {
    ...
    const raw = fs8.readFileSync(path8.join(kitRoot, "package.json"), "utf-8");
...
async function installHooks(targetDir) {
  ...
  if (!fs8.existsSync(path8.join(targetDir, ".git"))) {
...
async function deployEnvExample(targetDir, args = {}) {
  fs8.writeFileSync(path8.join(targetDir, ".env.example"), renderEnvFromTemplate(args));
}
async function deployEnv(targetDir, args = {}) {
  fs8.writeFileSync(path8.join(targetDir, ".env"), renderEnvFromTemplate(args));
}
...
async function deployVscodeSettings(targetDir, opts) {
  ...
  fs8.mkdirSync(destDir, { recursive: true });
  fs8.copyFileSync(src, path8.join(destDir, "settings.json"));
}
async function deployGitignore(targetDir, language = "java", opts) {
  const base = fs8.readFileSync(path8.join(commonDir(opts), ".gitignore.base"), "utf-8");
  ...
  fs8.writeFileSync(path8.join(targetDir, ".gitignore"), base + "\n" + extra);
}
...
function resolveAlembicBin(projectDir) {
  const candidates = [
    path15.join(projectDir, ".venv", "bin", "alembic"),
    path15.join(projectDir, "venv", "bin", "alembic")
  ];
...
function findVersionsDir(projectDir) {
  const candidates = [
    path16.join(projectDir, "migrations", "versions"),
    path16.join(projectDir, "alembic", "versions")
  ];
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir) {
    if (fs16.existsSync(path16.join(projectDir, "alembic.ini"))) return true;
    if (fs16.existsSync(path16.join(projectDir, "migrations", "env.py"))) return true;
    if (fs16.existsSync(path16.join(projectDir, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
function migrationsLocation(projectDir) {
  return `filesystem:${path17.join(projectDir, "src", "main", "resources", "db", "migration")}`;
}
...
async function statusFlyway(ctx) {
  ...
  for (const m of entries) {
    ...
    if (state === "SUCCESS" || state === "BASELINE") {
      ...
      const filename = m.filepath ? path17.basename(m.filepath) : `V${m.version}__migration.sql`;
      pending.push({
        version: m.version,
        filename,
        description: m.description ?? ""
...
function listFlywayFiles(projectDir) {
  const dir = path18.join(projectDir, "src", "main", "resources", "db", "migration");
...
var FlywayAdapter = {
  ...
  detect(projectDir) {
    return fs17.existsSync(path18.join(projectDir, "pom.xml"));
  },
  async apply(args) {
...
function findKnexfile(projectDir) {
  for (const name of KNEXFILE_VARIANTS) {
    const p = path19.join(projectDir, name);
...
var KnexAdapter = {
   ...
   */
  detect(projectDir) {
    return KNEXFILE_VARIANTS2.some((name) => fs19.existsSync(path20.join(projectDir, name)));
  },
  async apply(args) {
...
function detectLanguage(projectDir) {
  if (fs20.existsSync(path21.join(projectDir, "pom.xml"))) {
    return "java";
  }
  if (fs20.existsSync(path21.join(projectDir, "pyproject.toml")) || fs20.existsSync(path21.join(projectDir, "requirements.txt")) || fs20.existsSync(path21.join(projectDir, "alembic.ini"))) {
    return "python";
  }
  if (fs20.existsSync(path21.join(projectDir, "package.json"))) {
    return "nodejs";
  }
  throw new SchemaMigrationError(
...
import { readFileSync as readFileSync9, writeFileSync as writeFileSync9, existsSync as existsSync19, mkdirSync as mkdirSync9, readdirSync as readdirSync9, statSync as statSync4 } from "fs";
...
function readMasterTestList(tddDir, featureId) {
  ...
  const file = join21(dir, "test-list.json");
...
function findFeatureDir(tddDir, featureId) {
  const featuresDir = join21(tddDir, "features");
  if (!existsSync19(featuresDir)) {
    throw new Error(`${featuresDir} does not exist`);
  }
  const candidates = readdirSync9(featuresDir).filter((d) => d.startsWith(featureId));
  if (candidates.length === 0) {
    throw new Error(`feature ${featureId} not found under ${featuresDir}`);
  }
  return join21(featuresDir, candidates[0]);
}

// scripts/tdd/design-spec-gate.ts
import { appendFileSync, existsSync as existsSync22, readFileSync as readFileSync12, writeFileSync as writeFileSync11, mkdirSync as mkdirSync11 } from "fs";
import { dirname as dirname7, join as join24 } from "path";

// scripts/tdd/experiment.ts
import { existsSync as existsSync21, mkdirSync as mkdirSync10, readdirSync as readdirSync10, readFileSync as readFileSync11, statSync as statSync5, writeFileSync as writeFileSync10 } from "fs";
...
async function getEndpoint(args) {
  ...
  let raw;
  try {
    raw = execFileSync5("databricks", ["postgres", "list-endpoints", branchPath, "-o", "json"], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
      timeout: KIT_TIMEOUTS.cliDefault
    });
...
function gitCurrentBranch(cwd) {
  return execFileSync6("git", ["rev-parse", "--abbrev-ref", "HEAD"], {
    cwd,
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"],
...
function gitHasLocalBranch(cwd, branch) {
  try {
    execFileSync6("git", ["rev-parse", "--verify", "--quiet", `refs/heads/${branch}`], {
      cwd,
      stdio: "ignore",
      timeout: KIT_TIMEOUTS.gitDefault
    });
...
function gitCheckoutNewBranch(cwd, branch) {
  execFileSync6("git", ["checkout", "-b", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
...
function gitCheckoutExistingBranch(cwd, branch) {
  execFileSync6("git", ["checkout", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
    timeout: KIT_TIMEOUTS.gitCheckout
  });
...
function gitDeleteLocalBranch(cwd, branch, force = true) {
  execFileSync6("git", ["branch", force ? "-D" : "-d", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
    timeout: KIT_TIMEOUTS.gitDefault
  });
...
function gitDeleteRemoteBranch(cwd, remote, branch) {
  execFileSync6("git", ["push", remote, "--delete", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
  ...
  if (syncEnv && ready.state === "READY") {
    try {
      ...
      if (!ep?.host) {
        ...
        updateEnvConnection({
          envPath: path22.join(args.cwd, ".env"),
          branchId: sanitized,
          databaseUrl: dsn,
          username: email,
          password: token,
...
async function syncEnvToCurrentBranch(args) {
  const envPath = path22.join(args.cwd, ".env");
...
function listExperiments(tddDir, featureId) {
  const root = join23(tddDir, "experiments", featureId);
  if (!existsSync21(root)) return [];
  const out = [];
  for (const slug of readdirSync10(root)) {
    const dir = join23(root, slug);
    if (!statSync5(dir).isDirectory()) continue;
    const branchFile = join23(dir, "branch.txt");
    if (!existsSync21(branchFile)) continue;
    out.push({
      feature_id: featureId,
      experiment_slug: slug,
      branch_id: readFileSync11(branchFile, "utf8").trim(),
...
function readOutcomes(tddDir, featureId, slug) {
  const file = join23(tddDir, "experiments", featureId, slug, "outcomes.json");
...
function readPlan(tddDir, featureId) {
  const planPath = join24(tddDir, "features", `${featureId}`, "plan.json");
  if (!existsSync22(planPath)) return null;
  return JSON.parse(readFileSync12(planPath, "utf8"));
}

// scripts/tdd/smells.ts
import { existsSync as existsSync23, readFileSync as readFileSync13, writeFileSync as writeFileSync12 } from "fs";
import { join as join25 } from "path";
function readSmellsLog(tddDir) {
  const file = join25(tddDir, "smells.json");
  if (!existsSync23(file)) return { detected: [] };
  return JSON.parse(readFileSync13(file, "utf8"));
}

// scripts/tdd/gates.ts
import { existsSync as existsSync24, readFileSync as readFileSync14, readdirSync as readdirSync11, renameSync as renameSync2, unlinkSync as unlinkSync2, writeFileSync as writeFileSync13 } from "fs";
...
function gatesFilePath(tddDir, featureId) {
  return join26(findFeatureDir2(tddDir, featureId), "gates.json");
}
function findFeatureDir2(tddDir, featureId) {
  const featuresDir = join26(tddDir, "features");
  if (!existsSync24(featuresDir)) {
    throw new Error(`${featuresDir} does not exist`);
  }
  const candidates = readdirSync11(featuresDir).filter((d) => d.startsWith(featureId));
  if (candidates.length === 0) {
    throw new Error(`feature ${featureId} not found under ${featuresDir}`);
  }
  return join26(featuresDir, candidates[0]);
}
...
function readJsonIfExists(path25) {
  if (!existsSync25(path25)) return null;
  return JSON.parse(readFileSync15(path25, "utf8"));
}
function timelineCycleCount(experimentDir) {
  const timeline = readJsonIfExists(
    join27(experimentDir, "timeline.json")
  );
...
function readSelectionLogRecent(tddDir, limit) {
  const path25 = join27(tddDir, "selection-log.md");
  if (!existsSync25(path25)) return [];
  const text = readFileSync15(path25, "utf8");
  const entries = [];
...
function readWorkflowState(tddDir) {
  const state = readJsonIfExists(join27(tddDir, "workflow-state.json"));
...
function detectWorkflowDrift(args) {
  const projectWorkflowsDir = path23.join(
    args.projectDir,
    ".github",
    "workflows"
  );
  const here = path23.dirname(new URL(import.meta.url).pathname);
  const kitWorkflowsDir = args.kitDir ? path23.join(
    args.kitDir,
    "templates",
    "project",
    "common",
...
function checkEnv(projectDir) {
  ...
  if (Object.keys(env).length === 0) {
    return {
      ...
      detail: { projectDir, envPath: path24.join(projectDir, ".env") },
