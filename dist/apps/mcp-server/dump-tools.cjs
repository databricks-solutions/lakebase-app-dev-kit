...
function detectLanguage(projectDir) {
  if (fs14.existsSync(path14.join(projectDir, "pom.xml"))) {
    return "java";
  }
  if (fs14.existsSync(path14.join(projectDir, "pyproject.toml")) || fs14.existsSync(path14.join(projectDir, "requirements.txt")) || fs14.existsSync(path14.join(projectDir, "alembic.ini"))) {
    return "python";
  }
  if (fs14.existsSync(path14.join(projectDir, "package.json"))) {
    return "nodejs";
  }
  throw new SchemaMigrationError(
...
function findTemplatesDir4() {
  ...
  const here = path15.dirname((0, import_node_url4.fileURLToPath)(importMetaUrl));
  let dir = here;
  for (let i = 0; i < 6; i++) {
    const candidate = path15.join(dir, "templates", "project");
    if (fs15.existsSync(path15.join(candidate, "common", ".gitignore.base"))) {
...
function commonDir2(opts) {
  return path15.join(opts?.templatesDir ?? findTemplatesDir4(), "common");
}
var PLAYWRIGHT_TEMPLATE_FILES = [
  "playwright.config.ts",
  path15.join("tests", "e2e", "smoke.spec.ts")
];
function writePlaywrightTemplates(args) {
  ...
  for (const rel of PLAYWRIGHT_TEMPLATE_FILES) {
    const from = path15.join(src, rel);
    if (!fs15.existsSync(from)) {
      throw new Error(`Kit template missing: ${from}`);
    }
    const to = path15.join(args.projectDir, rel);
    if (fs15.existsSync(to) && !args.force) {
      skipped.push(rel);
      continue;
    }
    fs15.mkdirSync(path15.dirname(to), { recursive: true });
...
function addPlaywrightToPackageJson(args) {
  const pkgPath = path16.join(args.projectDir, "package.json");
...
function addE2eToRunTestsScript(args) {
  const scriptPath = path16.join(args.projectDir, "scripts", "run-tests.sh");
...
function enableE2eForProject(args) {
  const rootPkg = path16.join(args.projectDir, "package.json");
...
function cacheDir() {
  return path18.join(os.homedir(), ".cache", "github-actions-runner");
}
function runnersDir() {
  return path18.join(os.homedir(), ".lakebase", "runners");
}
function runnerDir(projectName) {
  return path18.join(runnersDir(), projectName);
}
...
async function ensureCachedArchive() {
  ...
  const cachedPath = path18.join(dir, RUNNER_ARCHIVE);
...
function stopRunner(projectName) {
  ...
  const pidFile = path18.join(dir, ".pid");
  ...
  for (const stale of ["_diag/pages", "_work/_temp", "_work/_actions"]) {
    const full = path18.join(dir, stale);
  ...
  try {
    fs18.mkdirSync(path18.join(dir, "_diag", "pages"), { recursive: true });
  } catch {
  }
}
function resetRunnerConfig(dir, projectName) {
  ...
  for (const f of stateFiles) {
    try {
      fs18.unlinkSync(path18.join(dir, f));
    } catch {
    }
  }
  if (process.platform === "darwin") {
    const plist = path18.join(
      os.homedir(),
      "Library",
      "LaunchAgents",
...
async function setupRunner(args) {
  ...
  if (child.pid) {
    fs18.writeFileSync(path18.join(dir, ".pid"), String(child.pid));
  }
...
function stateFilePath(projectDir) {
  return path19.join(projectDir, STATE_FILE_REL);
}
...
${summary}`);
  ...
  const dir = path19.join(projectDir, ".lakebase");
...
async function createProject(input, progress) {
  ...
  const projectDir = path20.join(input.parentDir, input.projectName);
...
function layDownTddScaffold(targetDir) {
  const candidates = [
    path20.resolve(__dirname, "../../templates/tdd-bootstrap/.tdd"),
    path20.resolve(__dirname, "../../../templates/tdd-bootstrap/.tdd")
  ];
  ...
  const dest = path20.join(targetDir, ".tdd");
...
function readMasterTestList(tddDir, featureId) {
  ...
  const file = (0, import_path2.join)(dir, "test-list.json");
...
function readPlan(tddDir, featureId, storyId) {
  const planPath = (0, import_path5.join)(tddDir, "features", featureId, "stories", storyId, "plan.json");
...
function gatesFilePath(tddDir, featureId) {
  return (0, import_path7.join)(findFeatureDir2(tddDir, featureId), "gates.json");
}
function findFeatureDir2(tddDir, featureId) {
  const featuresDir = (0, import_path7.join)(tddDir, "features");
  if (!(0, import_fs6.existsSync)(featuresDir)) {
    throw new Error(`${featuresDir} does not exist`);
  }
  const candidates = (0, import_fs6.readdirSync)(featuresDir).filter((d) => d.startsWith(featureId));
  if (candidates.length === 0) {
    throw new Error(`feature ${featureId} not found under ${featuresDir}`);
  }
  return (0, import_path7.join)(featuresDir, candidates[0]);
}
...
function listFeatureStories(tddDir, featureId) {
  const storiesDir = (0, import_path8.join)(tddDir, "features", featureId, "stories");
  if (!(0, import_fs7.existsSync)(storiesDir)) return [];
  return (0, import_fs7.readdirSync)(storiesDir).filter((d) => (0, import_fs7.statSync)((0, import_path8.join)(storiesDir, d)).isDirectory()).sort();
}
function timelineCycleCount(experimentDir2) {
  const timeline = readJsonIfExists(
    (0, import_path8.join)(experimentDir2, "timeline.json")
  );
...
function readSelectionLogRecent(tddDir, limit) {
  const path24 = (0, import_path8.join)(tddDir, "selection-log.md");
...
function readWorkflowState(tddDir) {
  const state = readJsonIfExists((0, import_path8.join)(tddDir, "workflow-state.json"));
