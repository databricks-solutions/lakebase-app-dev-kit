...
- A workflow skill's agent contract tells you to import it (e.g. the `lakebase-tdd-workflows` Architect Reviewer reviews every design against layering + twelve-factor; the Test Strategist authors the fitness functions named here).
...
| Mock policy | Mocks only where there's no real backing resource; the database is never mocked | "no-mock-for-DB" check (see [test-strategy](../lakebase-tdd-workflows/references/test-strategy.md)) |
...
- **`lakebase-tdd-workflows`** – Architect Reviewer imports this in architectural review (layering + twelve-factor); Test Strategist authors the fitness functions as RED tests; Navigator imports in PLAN; Driver keeps them green through REFACTOR.
