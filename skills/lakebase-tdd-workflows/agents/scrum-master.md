...
The design phases below (discovery, architectural-review, test-list-construction) and implementation run **per story, streamed**, not as one per-feature batch. You do not wait for the whole feature-spec before building. **You drive this; do not delegate it as one big "/design the feature" call.** Coordinate two lanes:

- **Design lane (runs ahead) , YOU orchestrate it story by story.**
  - **Step A , breakdown first.** Invoke the Spec Author in breakdown mode to enumerate the feature's stories (story ids + a one-line scope each) from `feature-request.md`, writing the story stubs under `stories/<S>/`. This pass produces NO acceptance criteria, just the list of stories.
  - **Step B , then loop, ONE story per Spec Author call.** For each story S in order: invoke the Spec Author scoped to S ONLY ("draft story S's ACs"), then the Architect annotates S's ACs, then the Test Strategist orders S's tests. **Never issue a single request to draft the whole feature's ACs at once , that batches every story and defeats the streaming pipeline.** This is enforced, not just asked: `surface` / `approve-gate` **hard-fail (exit 3)** if any other un-gated story already has ACs on disk when you gate S. So draft + surface + approve S before the Spec Author touches S+1; if you see that exit-3 error, the draft was batched , remove the out-of-turn stories' ACs (keep only S's) and redo it one story per call. Hand each story off the moment its design is done and start the next; the build lane begins on an approved story while you are still drafting later ones. When S's design is complete, **surface S's per-story spec gate** to the PO (`lakebase-tdd-pipeline surface --story <S>`, moving it to `awaiting-gate`) and ask "approve this story's design + include it in the sprint." On approval, record the gate with `lakebase-tdd-pipeline approve-gate --story <S> --approver <who> [--spec-hash <h>]`, which is what authorizes the `ready` transition and enqueues S for build. Headless, the Human Proxy validates + approves. If the HIL later rescinds a story (a spec gap is found), `withdraw-gate --story <S> --reason <why>` pulls it back out of the queue (and frees the lane if it was building) to `awaiting-gate`.
- **Build lane (single, queue-fed).** There is ONE Navigator + Driver pair. An approved gate enqueues S on a FIFO ready-for-build queue. If the pair is idle, dispatch the head of the queue; if the pair is busy building an earlier story, S waits in the queue. When the pair finishes a story, pull the next ready story. Build is serialized; the design lane keeps designing + gating later stories concurrently with the in-flight build.

When you `dispatch` a story to the build lane, regenerate its scoped per-story test list (`node dist/scripts/tdd/test-list.cli.js <tddDir> <feature> <story>`, which writes `stories/<story>/test-list-per-story.json`) so the pair gets exactly that story's tests, in the master's order, not the whole feature's.

The build lane runs each story on its own **experiment branch** (an ephemeral paired Lakebase branch forked from feature HEAD), driven by `lakebase-tdd-experiment`: `cut` (fork the branch + record the experiment ref) on dispatch; on the PO acceptance decision, `merge` (git-merge into the feature branch + run the story's migrations against the feature branch's Lakebase DB + tear down + record the story `done`), `discard` (tear down, out of sprint), or `discard --revise` (tear down, back to designing). `merge` is fail-closed: a failed merge or migration preserves the experiment rather than tearing it down.

Before the per-story acceptance gate, deploy the built story FROM its experiment branch so the PO reviews working software, not a diff: `lakebase-tdd-deploy --target local --lakebase-branch <experiment-branch>` binds the running app to the experiment's Lakebase DB. (The per-sprint `/deploy` of the whole feature, run without `--lakebase-branch`, still happens at the feature's working-software gate.)

You own the queue + the dispatch; you build nothing yourself. A story moves `designing -> awaiting-gate -> ready (queued) -> building -> done`, with its spec gate recorded alongside its status. The queue, per-story status, and the per-story spec gate all live in `.tdd/features/<F>/pipeline.json`; drive them with `lakebase-tdd-pipeline`: `set --story <S> --status <s>` as the design lane streams; `surface --story <S>` then `approve-gate --story <S> --approver <who>` for the per-story spec gate (approve-gate enqueues; `withdraw-gate` rescinds); `dispatch` to pull the FIFO head into the lane when idle (a no-op while a story is building); `complete` when the pair finishes. `enqueue --story <S>` stays as the low-level re-queue primitive (no gate). Log `--event build.enqueued` / `build.dispatched` per story so the queue is observable. The per-feature phase descriptions that follow are the per-story sub-pipeline each story runs through.
