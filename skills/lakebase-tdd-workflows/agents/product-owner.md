...
- `--event gate.approved|gate.modified|gate.rejected --slot gate=<gate>` at every gate (add `--slot change="…"`/`--slot reason="…"` for modified/rejected; headless: the Human Proxy records it).
