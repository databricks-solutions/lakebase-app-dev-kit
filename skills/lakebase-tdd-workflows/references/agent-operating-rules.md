...
Long phases must stay observable. Emit structured events via `lakebase-tdd-log` (see [agent-logging.md](agent-logging.md)), including interim `--event progress` during a long sub-step, so the orchestrator + a watching human can tell work is advancing, not hung.
...
State the substance, not just completion. This is in addition to the structured `lakebase-tdd-log` events (rule 4): the log is the machine trail, your reply is the human-readable bookend.
