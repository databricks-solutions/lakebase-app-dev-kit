...
import { recordBlockingSmellFlag } from "../../scripts/tdd/escalation";
import { nextTransition, actionLane, type DriveState } from "../../scripts/tdd/orchestrator-drive";
...
import { decideEscalationAsHumanProxy } from "../../scripts/tdd/human-proxy";
import { writePipeline, readPipeline, type StoryPipeline } from "../../scripts/tdd/story-pipeline";
...
describe("commandsForAction(revise-route) (FEIP-7626)", () => {
  it("emits a single human-proxy decide-escalation carrying story/smell/route/verdict", () => {
    ...
    if (c.kind === "cli") {
      expect(c.bin).toBe("lakebase-tdd-human-proxy");
