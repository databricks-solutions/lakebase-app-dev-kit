# tier hierarchy + enforced TDD-workflow handoff contracts

**Ticket:**, a sub-task of (umbrella: SCM + TDD workflows as
...
- Thin scaffolded shells onto TS substrate CLIs: mostly landed;
...
- Live tests must exercise scaffolded touchpoints, not bypass
  ...
  remaining 7495 work.
Builds on (SCM workflow state machine); surfaced by the TDD-workflow smoke.
...
the source). This work does NOT introduce new contract; it ENFORCES what
...
- **Umbrella §2 (`feature-claimed` postconditions)** already require "**Git HEAD is on
  ...
  going unenforced. build-order item 8 ("make gates BLOCKING + retire
