...
`/lakebase-app-dev-kit:tdd` is one smart command: in a scaffolded `.tdd/` project it takes stock and resumes the `/plan -> /design -> /build -> /deploy` loop; elsewhere it guides project creation (`lakebase-create-project`), then resumes. The slash commands invoke the deterministic orchestrator (`lakebase-sftdd-drive`), which spawns the role agents. (Superseded by `orchestrator-deterministic-driver.md`: the orchestrator is now a deterministic driver, not an `--agent scrum-master` session.) NOT validated live yet (`/plugin marketplace add` + install + cross-reference resolution in plugin agents) , that is a manual smoke.
